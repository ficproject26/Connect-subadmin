...
export function StateKYC() {
    ...
    },
    {
      ...
      render: (row) => (
        <button
          ...
          className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-300 dark:hover:border-blue-700 transition cursor-pointer inline-flex items-center justify-center"
        >
  ...
  return (
    <div className="space-y-4">
      {/* Page Header */}
      <div>
        <h2 className="text-lg font-bold text-slate-900 dark:text-white">State KYC Compliance Desk</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Vendor identity verification clearance and document verification roster across the State.
        </p>
      </div>
      ...
      <DataTable
        ...
        subtitle="Review identity documents and compliance verification for vendors"
        columns={columns}
        data={kycList}
        loading={loading}
        onRefresh={loadData}
        ...
        exportFileName="state_vendor_kyc.csv"
      />
