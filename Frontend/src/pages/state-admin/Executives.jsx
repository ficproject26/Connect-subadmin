...
export function StateExecutives() {
  ...
  return (
    <div className="space-y-4">
      {/* Page Header */}
      <div>
        <h2 className="text-lg font-bold text-slate-900 dark:text-white">State Vendor Executives</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Vendor-assigned Stay and Travel executives managing customer bookings and operations across the State.
        </p>
      </div>

      {/* 4 KPI Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {/* KPI 1: Total Executives */}
        <div className="p-3 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
          ...
          <div className="text-[10px] text-indigo-600 dark:text-indigo-400 font-medium mt-0.5">
            Affiliated vendor personnel
          </div>
        </div>

        {/* KPI 2: Stay Executives */}
        <div className="p-3 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
          <div className="flex items-center justify-between">
          ...
          <div className="text-[10px] text-amber-600 dark:text-amber-400 font-medium mt-0.5">
            Resorts, hotels &amp; rooms
          </div>
        </div>

        {/* KPI 3: Travel Executives */}
        <div className="p-3 rounded-xl bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
          <div className="flex items-center justify-between">
      ...
      <DataTable
        ...
        subtitle="Vendor-assigned personnel handling stay, tour, and travel operations"
        columns={columns}
        data={execs}
        loading={loading}
        onRefresh={loadData}
        onRowClick={(row) => setSelectedExecutive(row)}
        searchPlaceholder="Search executive name, vendor, shop, or pincode..."
        exportFileName="state_executives_roster.csv"
      />
