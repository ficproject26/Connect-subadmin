...
export function StateDivisionAdmins() {
      ...
      render: (row) => (
        <div>
          ...
          <span className={`text-[10px] font-mono ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            {row.district} District • Code: {row.code}
          </span>
        </div>
      )
    },
    ...
    },
    {
      ...
      render: (row) => (
        <button
          ...
          title="View Division Admin Profile"
        >
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>State Division Administrators</h2>
        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          Supervisory personnel and nodal officers assigned to divisional jurisdictions across {user?.state || 'Tamil Nadu'}.
        </p>
      </div>
