  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            {districtFilter ? (
              <span>Filtered by District: <strong className="text-blue-600 font-bold">{districtFilter}</strong>. Click a row to view Admin details, or "View Pincodes" to drill down.</span>
            ) : (
              <span>Overview of all administrative divisions operating across this State. Click a row to view Admin details.</span>
            )}
          </p>
        </div>
      ...
      <DataTable
        ...
        subtitle="Hierarchical administration divisions under state jurisdiction. Click any row to inspect Admin details."
        columns={columns}
        data={divisions}
        loading={loading}
        onRefresh={loadData}
