...
export function DistrictJobs() {
      ...
      render: (row) => (
        <div className="text-xs whitespace-nowrap">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 pl-4">
            {row.district || user?.district || '-'} / {row.division || '-' }
          </div>
        </div>
      )
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">District Job Applications</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Applications submitted for vendor jobs across divisions in this District.</p>
      </div>
      ...
      <DataTable
        title="District Applications Board"
        subtitle="Customer applications for vendor job vacancies"
        columns={columns}
        data={jobs}
        loading={loading}
        onRefresh={loadData}
