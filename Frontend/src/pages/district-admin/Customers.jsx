      ...
      render: (row) => (
        <div className="space-y-0.5">
          <div className={`font-semibold text-xs ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
            {row.district ? `${row.district} / ` : ''}{row.division}
          </div>
    ...
    <div className="space-y-6">
      <div>
        <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>
          District Customer Directory
        </h2>
        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          All registered customers within assigned district jurisdiction, including card and non-card members.
        </p>
      </div>
