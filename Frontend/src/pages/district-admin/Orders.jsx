...
export function DistrictOrders() {
  ...
  const kpiStats = useMemo(() => {
    ...
    if (total > 0) {
      const divisionCounts = orders.reduce((acc, o) => {
        const div = o.division || o.district || districtName;
        acc[div] = (acc[div] || 0) + 1;
        return acc;
      }, {});
      ...
      render: (row) => (
        <div className="whitespace-nowrap">
          <div className={`text-xs font-medium ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
            {row.district || districtName} / {row.division}
          </div>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>District Orders Overview</h2>
        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          All customer orders dispatched and delivered across {districtName} District.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        ...
        {/* KPI 4: Order Peak – Highest Ordering Division */}
        <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
          <div className="flex items-center justify-between">
          ...
          <div className="text-[10px] text-amber-600 dark:text-amber-400 font-medium mt-0.5 truncate">
            Leading division ({kpiStats.peakCount} orders • {kpiStats.peakPercent}%)
          </div>
        </div>
      </div>
      ...
      <DataTable
        title="Orders Ledger"
        subtitle="Live orders from all district divisions and pincodes"
        columns={columns}
        data={orders}
        loading={loading}
        onRefresh={loadData}
      ...
      {/* Order Details Modal with Status Tracking Timeline */}
      <OrderDetailsModal
        order={selectedOrder}
