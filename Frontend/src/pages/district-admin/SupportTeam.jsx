...
export function DistrictSupportTeam() {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">District Customer Support Queue</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Customer grievances and technical requests originating from {user?.district ? `${user.district} District` : 'assigned district'}.
        </p>
      </div>

      <DataTable
        title="Support Desk Queue"
        subtitle="Manage localized customer inquiries and escalation handling"
        columns={columns}
        data={tickets}
        loading={loading}
        onRefresh={loadData}
        searchPlaceholder="Search ticket ID or subject..."
        exportFileName="district_support_tickets.csv"
      />
    </div>
  );
}
