...
export function DivisionalSettings() {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Divisional Admin Settings</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">Security preferences, alert configurations, and operational parameters.</p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-5">
        <div className="admin-card p-6 bg-white dark:bg-[#131f37] border border-slate-200/90 dark:border-[#1f3358] shadow-sm space-y-4">
          <div className="flex items-center gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
            ...
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Divisional Officer Profile</h3>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            ...
            <div>
              <label className="block text-slate-600 dark:text-slate-400 mb-1 font-medium">Assigned Division</label>
              <input
                type="text"
                disabled
        ...
        <div className="admin-card p-6 bg-white dark:bg-[#131f37] border border-slate-200/90 dark:border-[#1f3358] shadow-sm space-y-4">
          <div className="flex items-center gap-3 pb-3 border-b border-slate-100 dark:border-slate-800">
            ...
            <h3 className="text-sm font-bold text-slate-900 dark:text-white">Divisional Notifications</h3>
          </div>
          ...
          <div className="space-y-3 text-xs">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input type="checkbox" defaultChecked className="w-4 h-4 text-blue-600 rounded" />
              <span className="text-slate-700 dark:text-slate-300 font-medium">
                Alert on pending payments requiring clearance
              </span>
            </label>
          </div>
        </div>
