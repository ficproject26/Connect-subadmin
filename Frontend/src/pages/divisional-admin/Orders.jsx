...
export function DivisionalOrders() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>Division Orders Overview</h2>
        <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
          All customer orders dispatched and delivered across {divisionName} Division.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        ...
        {/* KPI 4: Order Peak – Highest Ordering Pincode */}
        <div className="p-3.5 rounded-2xl bg-white dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800/80 shadow-sm">
          <div className="flex items-center justify-between">
          ...
          <div className="text-lg font-bold text-slate-900 dark:text-white mt-1.5 truncate">
            PIN: {kpiStats.peakLocation}
          </div>
          <div className="text-[10px] text-amber-600 dark:text-amber-400 font-medium mt-0.5 truncate">
            Leading pincode ({kpiStats.peakCount} orders &bull; {kpiStats.peakPercent}%)
          </div>
        </div>
      </div>
      ...
      <DataTable
        title="Orders Ledger"
        subtitle="Live orders from assigned division pincodes"
        columns={columns}
        data={orders}
        loading={loading}
        onRefresh={loadData}
