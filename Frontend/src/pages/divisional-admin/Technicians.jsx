...
export function DivisionalTechnicians() {
  const { user } = useAuth();
      ...
      render: (row) => (
        <div className="text-xs">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium pl-4.5 mt-0.5">
            {row.district ? `${row.district}${row.division ? `, ${row.division}` : ''}` : `${user?.division || 'Division'} Zone`}
          </div>
        </div>
      )
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        subtitle="Manage service crew readiness and dispatch across wards"
        columns={columns}
        data={techs}
        loading={loading}
        onRefresh={loadData}
        ...
        exportFileName="divisional_technicians.csv"
      />
