      ...
      render: (row) => (
        <div className="text-xs">
          ...
          <div className="text-[11px] text-slate-500 dark:text-slate-400 font-medium pl-4.5 mt-0.5">
            {row.district ? `${row.district}${row.division ? `, ${row.division}` : ''}` : `PIN: ${user?.pincode || 'Local Zone'}`}
          </div>
        </div>
      )
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        subtitle="Manage service personnel readiness and local assignments"
        columns={columns}
        data={techs}
        loading={loading}
        onRefresh={loadData}
        onRowClick={(row) => setSelectedTechnician(row)}
        searchPlaceholder="Search technician name, phone, or skill..."
